...
// Problem 1
//Create email marketing listed

let marketingList = [];

marketingList = runners.map(function(runners) {
  return (`${runners.id} ${runners.email}`);
});
...
// Problem 2
// Show only runners with ID under 20 for first start group.
let firstGroup = [];

firstGroup = runners.filter(function(runners) {
  return runners.id <= 20;
});
console.log(firstGroup);


// Problem 3
// See donations by company

let donationsCompany = [];

runners.forEach(function(runners) {
   donationsCompany.push(`${runners.company_name} ${runners.donation}`);
});

console.log(donationsCompany);
